...
var notificationHandler = require('./notification_handler');
...
// Event Handling
IDAttributes.$events = function(node, name, listeners) {
  if (name !== '$events') {
    return;
  }
  for (var eventName in listeners) {
    ...
    for (var i = 0, l = callbacks.length; i < l; i++) {
      node.addEventListener(eventName, callbacks[i]);
    }
  }
};
...
function CremeElement(tagName, isVoid, attributes, children) {
  ...
  /**
   * The associated node with this element.
   *
   * @type {Element|Text}
   */
  this.node = null;
...
CremeElement.prototype._init = function() {
  ...
  if (attributes.$id) {
    ...
    this._static.push('id', id);
    attributes.$id = INTERNAL;
    attributes.$ref = true;
    // Hacky - Subscribe to notifications to get data.
  }
  if (attributes.$ref) {
    if (!this._key) {
      this._key = ('ref-id-' + uid++);
    }
    notificationHandler.add(this._key, function(node, removed) {
      this.node = removed ? null : node;
    }.bind(this));
    attributes.$ref = INTERNAL;
  }
...
CremeElement.prototype._render = function(data) {
  ...
  if (children && children.length) {
    this._renderItem(children, data);
  }
  elementClose(tagName);
  return element;
};
